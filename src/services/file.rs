...
pub fn edit_task(config: &Config, id: usize, content: String) {
    match get_file_data(&config) {
        Ok((mut data, index)) => {
...
pub fn add_task(config: &Config, task: String) {
    match get_file_data(&config) {
        Ok((mut data, index)) => {
            let date = Local::now();
...
pub fn del_task(config: &Config, args: String) {
    match get_file_data(&config) {
        Ok((mut data, index)) => {
...
pub fn list_tasks(config: &Config, all: bool) {
    match get_file_data(&config) {
        Ok((data, index)) => {
            if all {
                for ctx in &data {
...
pub fn mark_done(config: &Config, args: String) {
    match get_file_data(&config) {
        Ok((mut data, index)) => {
            let ids = parse_ids(parse_args(&args));
...
pub fn clear_tasks(config: &Config) {
    match get_file_data(&config) {
        Ok((mut data, index)) => {
...
pub fn del_context(config: &Config, args: String) {
    match get_file_data(&config) {
        Ok((data, _)) => {
...
pub fn list_contexts(config: &Config) {
    match get_file_data(&config) {
        Ok((data, _)) => {
